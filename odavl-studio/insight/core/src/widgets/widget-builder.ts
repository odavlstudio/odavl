...
export class WidgetBuilder extends EventEmitter {
  ...
  createFromTemplate(templateId: string): WidgetDefinition {
    ...
    // Phase 3B: Use structuredClone() instead of JSON.parse(JSON.stringify())
    this.definition = structuredClone(template.definition);
   ...
   */
  clone(): WidgetBuilder {
    // Phase 3B: Use structuredClone() for deep copy
    const cloned = new WidgetBuilder(structuredClone(this.definition));
...
`.trim();
  ...
  save(version?: string, changelog?: string): WidgetRevision {
    const revision: WidgetRevision = {
      ...
      // Phase 3B: Use structuredClone() for snapshot
  ...
  restoreRevision(revisionId: string): void {
    ...
    // Phase 3B: Use structuredClone() for revision restore
    this.definition = structuredClone(revision.definition);
  ...
  getDefinition(): WidgetDefinition {
    // Phase 3B: Use structuredClone() for export
