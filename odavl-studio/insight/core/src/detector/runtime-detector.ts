...
export interface RuntimeError {
    ...
    file: string;
    line?: number;
    errorType: 'unhandled-promise' | 'uncaught-exception' | 'crash' | 'assertion-failure' | 'memory-error' |
    'memory-leak' | 'event-listener-leak' | 'interval-leak' | 'timeout-leak' |
    'race-condition' | 'resource-not-cleaned' | 'websocket-leak' | 'db-connection-leak';
    message: string;
    stack?: string;
    rootCause: string;
    suggestedFix: string;
    severity: 'critical' | 'high' | 'medium' | 'low';
    confidence?: number; // 0-100 - confidence score for this detection
}

export class RuntimeDetector {
    private readonly workspaceRoot: string;
    private readonly enhancedDbDetector: EnhancedDBDetector;
    ...
    constructor(workspaceRoot: string) {
        ...
        this.memoryLeakDetector = new MemoryLeakDetector(workspaceRoot);
        this.raceConditionDetector = new RaceConditionDetector(workspaceRoot);
        this.resourceCleanupDetector = new ResourceCleanupDetector(workspaceRoot);
    }
    ...
    private async detectDBConnectionLeaks(targetDir?: string): Promise<RuntimeError[]> {
        try {
            ...
            logger.error('[RuntimeDetector] Enhanced DB detection error:', error);
...
componentWillUnmount() {
  ...
  element.removeEventListener('event', this.handler);
     ...
     * Phase 3 Enhancement
     */
    private async detectRaceConditions(dir: string): Promise<RuntimeError[]> {
        const errors: RuntimeError[] = [];
...
await mutex.runExclusive(async () => {
  // synchronized access
     ...
     * Phase 3 Enhancement
     */
    private async detectResourceCleanupIssues(dir: string): Promise<RuntimeError[]> {
        const errors: RuntimeError[] = [];
...
await pipeline(
  ...
  fs.createWriteStream(outputPath)
     ...
     */
    private async parseLogFile(logPath: string): Promise<RuntimeError[]> {
        const errors: RuntimeError[] = [];
        ...
        const patterns = [
            ...
            },
            {
                ...
                type: 'uncaught-exception' as const,
                severity: 'critical' as const
            },
            {
     ...
     */
    private async scanCodeForRuntimeIssues(dir: string): Promise<RuntimeError[]> {
        const errors: RuntimeError[] = [];
     ...
     */
    private findCriticalAsyncIssues(content: string, filePath: string): RuntimeError[] {
        const errors: RuntimeError[] = [];
   ...
   (async () => {
     ...
     */
    private isInTryBlock(lines: string[], lineIndex: number): boolean {
        let openBraces = 0;
        let inTry = false;
     ...
     */
    private getRootCause(errorType: RuntimeError['errorType'], message: string): string {
        const causes: Partial<Record<RuntimeError['errorType'], string>> = {
     ...
     */
    private getSuggestedFix(errorType: RuntimeError['errorType'], file: string): string {
        const fixes: Partial<Record<RuntimeError['errorType'], string>> = {
   ...
   - Use streaming instead of loading all data`,
    ...
    /**
     * Convert MemoryLeakIssue to RuntimeError format
     */
    private convertMemoryLeaksToRuntimeErrors(issues: any[]): RuntimeError[] {
        return issues.map(issue => ({
            timestamp: new Date().toISOString(),
            file: issue.file,
            line: issue.line,
            errorType: issue.type as RuntimeError['errorType'],
            message: issue.message,
            rootCause: issue.rootCause,
            suggestedFix: issue.suggestedFix,
    ...
    /**
     * Convert RaceConditionIssue to RuntimeError format
     */
    private convertRaceConditionsToRuntimeErrors(issues: any[]): RuntimeError[] {
        return issues.map(issue => ({
            timestamp: new Date().toISOString(),
            file: issue.file,
            line: issue.line,
    ...
    /**
     * Convert ResourceCleanupIssue to RuntimeError format
     */
    private convertResourceIssuesToRuntimeErrors(issues: any[]): RuntimeError[] {
        return issues.map(issue => ({
            timestamp: new Date().toISOString(),
            file: issue.file,
            line: issue.line,
            errorType: 'resource-not-cleaned' as const,
            message: issue.message,
            rootCause: issue.rootCause,
            suggestedFix: issue.suggestedFix,
            severity: issue.severity,
     ...
     */
    formatError(error: RuntimeError): string {
        const relPath = path.relative(this.workspaceRoot, error.file);
        let emoji = '⚠️';
        if (error.severity === 'critical') emoji = '💥';
        else if (error.severity === 'high') emoji = '🔥';
        else if (error.severity === 'low') emoji = 'ℹ️';

        // Build confidence display with color-coded indicator
        let confidenceStr = '';
        if (error.confidence !== undefined) {
            let confidenceIndicator = '🔴'; // Low (<50%)
            if (error.confidence >= 80) confidenceIndicator = '🟢'; // High (≥80%)
            else if (error.confidence >= 50) confidenceIndicator = '🟡'; // Medium (50-79%)
