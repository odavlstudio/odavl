...
export enum NetworkErrorType {
    // API monitoring
    FETCH_WITHOUT_ERROR_HANDLING = 'FETCH_WITHOUT_ERROR_HANDLING',
    AXIOS_WITHOUT_INTERCEPTOR = 'AXIOS_WITHOUT_INTERCEPTOR',
    MISSING_TIMEOUT = 'MISSING_TIMEOUT',
    HARDCODED_URL = 'HARDCODED_URL',

    // Timeout issues
    NO_REQUEST_TIMEOUT = 'NO_REQUEST_TIMEOUT',
    EXCESSIVE_TIMEOUT = 'EXCESSIVE_TIMEOUT',
    MISSING_ABORT_CONTROLLER = 'MISSING_ABORT_CONTROLLER',

    // Error handling
    MISSING_RETRY_LOGIC = 'MISSING_RETRY_LOGIC',
    NO_FALLBACK_MECHANISM = 'NO_FALLBACK_MECHANISM',
    UNHANDLED_NETWORK_ERROR = 'UNHANDLED_NETWORK_ERROR',

    // Concurrency
    CONCURRENT_REQUESTS_WITHOUT_LIMIT = 'CONCURRENT_REQUESTS_WITHOUT_LIMIT',
    RACE_CONDITION_RISK = 'RACE_CONDITION_RISK',
    PROMISE_ALL_WITHOUT_ERROR_HANDLING = 'PROMISE_ALL_WITHOUT_ERROR_HANDLING',
}
...
export class NetworkDetector {
     ...
     */
    async detect(targetDir?: string): Promise<NetworkError[]> {
     ...
     */
    private detectFetchIssues(content: string, filePath: string): NetworkError[] {
        const errors: NetworkError[] = [];
...
fetch(\`\${API_URL}/endpoint\`)`,
     ...
     */
    private detectAxiosIssues(content: string, filePath: string): NetworkError[] {
        const errors: NetworkError[] = [];
        const lines = content.split('\n');
...
api.interceptors.response.use(
  ...
  error => {
     ...
     */
    private detectTimeoutIssues(content: string, filePath: string): NetworkError[] {
        const errors: NetworkError[] = [];
        const lines = content.split('\n');
...
- For long operations: Consider WebSocket or Server-Sent Events`,
     ...
     */
    private detectErrorHandlingIssues(content: string, filePath: string): NetworkError[] {
        const errors: NetworkError[] = [];
        const lines = content.split('\n');
...
const data = await response.json();`,
     ...
     */
    private detectConcurrencyIssues(content: string, filePath: string): NetworkError[] {
        const errors: NetworkError[] = [];
        const lines = content.split('\n');
...
useEffect(() => {
  ...
  return () => {
     ...
     */
    private shouldExclude(filePath: string, content: string): boolean {
        ...
        const typeLines = lines.filter(line =>
            /^\s*(export\s+)?(interface|type|enum)\s+/.test(line)
        ).length;
     ...
     */
    getStatistics(errors: NetworkError[]): NetworkStatistics {
        const stats: NetworkStatistics = {
            totalIssues: errors.length,
            bySeverity: { critical: 0, high: 0, medium: 0, low: 0 },
            byType: {} as Record<NetworkErrorType, number>,
        ...
        for (const type of Object.values(NetworkErrorType)) {
            stats.byType[type as NetworkErrorType] = 0;
        }
     ...
     */
    formatError(error: NetworkError): string {
...
`;
     ...
     */
    private getTypeEmoji(type: NetworkErrorType): string {
