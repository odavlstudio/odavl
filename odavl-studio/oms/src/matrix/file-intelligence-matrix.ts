...
export function buildFileIntelligenceMatrix(): FileIntelligenceProfile[] {
  return [
    // Code files
...
export function getFileIntelligence(typeId: string): FileIntelligenceProfile | undefined {
  const matrix = buildFileIntelligenceMatrix();
  return matrix.find((profile) => profile.typeId === typeId);
}
