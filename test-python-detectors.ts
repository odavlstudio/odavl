 ...
 * Manual test script using tsx (TypeScript execution)
 */
...
async function testPythonDetectors() {
    ...
    try {
        ...
        if (complexityIssues.length > 0) {
            console.log('Sample issues:');
            complexityIssues.slice(0, 5).forEach(issue => {
                console.log(`  - ${issue.severity.toUpperCase()}: ${issue.message}`);
                console.log(`    File: ${issue.file}:${issue.line}`);
            });
        }
    ...
    try {
        ...
        if (importsIssues.length > 0) {
            console.log('Sample issues:');
            importsIssues.slice(0, 5).forEach(issue => {
                console.log(`  - ${issue.severity.toUpperCase()}: ${issue.message}`);
                console.log(`    File: ${issue.file}:${issue.line}`);
            });
        }
    ...
    try {
        ...
        if (practicesIssues.length > 0) {
            console.log('Sample issues:');
            practicesIssues.slice(0, 5).forEach(issue => {
                console.log(`  - ${issue.severity.toUpperCase()}: ${issue.message}`);
                console.log(`    File: ${issue.file}:${issue.line}`);
            });
        }
