...
export class ODAvlRealtimeAnalyticsEngine extends EventEmitter {
  ...
  private readonly dataBuffer: Map<string, TimeSeriesDataPoint[]> = new Map();
  ...
  private handleUnsubscription(clientId: string, preferences: Partial<SubscriptionPreferences>): void {
    ...
    if (preferences.rooms) {
      for (const room of preferences.rooms) {
        this.leaveRoom(clientId, room);
      }
    }
  ...
  private aggregateDataPoints(dataPoints: TimeSeriesDataPoint[]): TimeSeriesDataPoint {
