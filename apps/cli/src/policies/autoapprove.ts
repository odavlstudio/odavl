 ...
 */
function loadAutoApprovalPolicy(): AutoApprovalPolicy {
 ...
 * ```
 */
export function evaluateCommandApproval(command: string): ApprovalResult {
  const policy = loadAutoApprovalPolicy();
 ...
 */
export function logApprovalDecision(command: string, result: ApprovalResult, phase: string = "DECIDE") {
  const policy = loadAutoApprovalPolicy();
 ...
 */
function writeAuditTrail(logData: Record<string, unknown>) {
  const ROOT = process.cwd();
 ...
 */
export function validatePolicy(): { valid: boolean; issues: string[] } {
  const issues: string[] = [];
  ...
  try {
    ...
    for (const required of requiredDenyPatterns) {
      const hasDenyRule = policy.deny.some(rule => {
