...
// Helper function for trust score badge variants
function getTrustVariant(trust: number): "default" | "secondary" | "destructive" {
  if (trust >= 0.8) return 'default';
  if (trust >= 0.6) return 'secondary';
  return 'destructive';
}

// Helper function for trust score colors
function getTrustColor(trust: number): string {
  if (trust >= 0.8) return 'bg-green-500';
  if (trust >= 0.6) return 'bg-yellow-500';
  return 'bg-red-500';
}
              ...
              {data.trust.sort((a, b) => b.trust - a.trust).map((recipe) => (
                <div key={recipe.id} className="p-4 bg-slate-50 rounded-lg border">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Badge 
                      variant={getTrustVariant(recipe.trust)}
                      className="text-xs"
                    >
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div 
                      className={`h-2 rounded-full transition-all duration-500 ${getTrustColor(recipe.trust)}`}
